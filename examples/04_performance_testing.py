...
class HighVolumeAccount(Aggregate):
    ...
    def record_transaction(self, from_account: str, to_account: str, amount: float, 
...
async def test_high_volume_event_creation():
    ...
    return accounts, PerformanceMetrics("Event Creation", duration, throughput, total_events)
...
async def test_bulk_persistence():
    ...
    total_events = sum(len(acc.get_uncommitted_events()) for acc in accounts) + sum(acc.version for acc in accounts)
    throughput = total_events / duration
    ...
    return event_store, accounts, PerformanceMetrics("Bulk Persistence", duration, throughput, total_events)
...
async def test_event_loading_performance():
    ...
    print(f"   ✓ Single aggregate: {len(single_events)} events in {single_duration*1000:.2f}ms")
    ...
    print(f"   ✓ Bulk loading: {total_loaded} events from {len(original_accounts)} aggregates")
    print(f"   ✓ Loading time: {bulk_duration:.3f} seconds")
    print(f"   ✓ Throughput: {throughput:.0f} events/second")
    
    return all_events, PerformanceMetrics("Event Loading", bulk_duration, throughput, total_loaded)
...
async def test_aggregate_reconstruction():
    ...
    successful_reconstructions = len([acc for acc in reconstructed_accounts if acc is not None])
    
    throughput = total_events_processed / duration if duration > 0 else 0
    
    print(f"   ✓ Reconstructed {successful_reconstructions}/{len(all_events)} aggregates")
    print(f"   ✓ Processed {total_events_processed} events in {duration:.3f} seconds") 
    print(f"   ✓ Throughput: {throughput:.0f} events/second")
    if successful_reconstructions > 0:
        print(f"   ✓ Average: {duration/successful_reconstructions*1000:.2f}ms per aggregate")
    
    return reconstructed_accounts, PerformanceMetrics("Reconstruction", duration, throughput, total_events_processed)
...
async def test_memory_efficiency():
    ...
    print(f"   ✓ Memory pattern: {event_count} iterations → {uncommitted_count} uncommitted events")
    ...
    return PerformanceMetrics("Memory Efficiency", batch_time, event_count/batch_time, event_count)
...
async def run_performance_benchmark():
    ...
    for metric in metrics:
        print(f"{metric.operation:<20} {metric.duration:<12.3f} "
