...
class Product:
    ...
    def release_stock(self, quantity: int, order_id: str, reason: str):
        ...
        return InventoryReleased(
            product_id=self.id, quantity=quantity, order_id=order_id, reason=reason
        )
...
class OrderProcessingService:
    ...
    async def process_order(self, order: Order) -> Dict[str, Any]:
        ...
        results = {"order_id": order.id, "status": "started", "steps": [], "errors": []}
        ...
        try:
            ...
            try:
                ...
                results["steps"].append(
                    f"Customer validated: {user.name} ({user.email})"
                )
            ...
            if not product.can_reserve(order.quantity):
                raise ValueError(
                    f"Insufficient stock for {order.product_id}: available {product.available_stock}, requested {order.quantity}"
                )

            product.reserve_stock(order.quantity, order.id, order.customer_id)
            results["steps"].append(
                f"Reserved {order.quantity} units of {product.name}"
            )
            ...
            results["steps"].append(
                f"Inventory fulfilled - remaining stock: {product.total_stock}"
            )
                ...
                if order.product_id in self.products:
                    ...
                    if product.reserved_stock > 0:
                        product.release_stock(
                            order.quantity, order.id, f"Order failed: {e}"
                        )
                        results["steps"].append(
    ...
    async def setup_test_users(self) -> Dict[str, User]:
        ...
        inactive_reg = UserRegistered(
            name="Inactive User", email="inactive@example.com"
        )
...
async def demonstrate_multi_aggregate_coordination():
    ...
    print("   ✓ Initialized product inventory")
    inventory = service.get_inventory_status()
    for pid, info in inventory.items():
        print(
            f"     - {info['name']}: ${info['price']:.2f}, stock: {info['available_stock']}"
        )
    ...
    for pid, info in final_inventory.items():
        print(
            f"     - {info['name']}: {info['available_stock']} available, "
            f"{info['reserved_stock']} reserved, {info['total_stock']} total"
        )

    print("\n   Order Summary:")
    print(
        f"     - Successful orders: {sum(1 for r in [result1, result2, result3] + results if not isinstance(r, Exception) and r['status'] == 'completed')}"
    )
    print(
        f"     - Failed orders: {sum(1 for r in [result1, result2, result3] + results if not isinstance(r, Exception) and r['status'] == 'failed')}"
    )
...
async def main():
    await demonstrate_multi_aggregate_coordination()

    print("\n✅ SUCCESS! Multi-aggregate coordination patterns demonstrated!")

    print("\nKey patterns demonstrated:")
    print("- ✓ Cross-aggregate business process orchestration")
    print("- ✓ Inventory reservation and release coordination")
    print("- ✓ Customer validation across aggregate boundaries")
    print("- ✓ Compensating actions for failed transactions")
    print("- ✓ Concurrent order processing with resource contention")
    print("- ✓ Multi-step workflow with rollback capabilities")
    print("- ✓ Event-driven coordination without tight coupling")
