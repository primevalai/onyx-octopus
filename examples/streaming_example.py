...
from eventuali import (
    EventStore, Event, EventStreamer, Subscription, SubscriptionBuilder,
    Projection, StreamEvent
)
...
class UserProjection(Projection):
    ...
    async def handle_event(self, event: Event) -> None:
        ...
        if event.event_type == "UserRegistered":
            ...
            print(f"[Projection] User registered: {event.data['name']} ({event.data['email']})")
            ...
            if user_id in self.users:
                ...
                print(f"[Projection] Email changed for user {user_id}: {old_email} -> {event.data['new_email']}")
...
async def main():
    ...
    async def process_events():
        ...
        try:
            async for stream_event in receiver:
                ...
                await user_projection.set_last_processed_position(stream_event.global_position)
                print(f"[Background] Processed event at position {stream_event.global_position}")
    ...
    email_change_event = UserEmailChanged("user-1", "alice@example.com", "alice.smith@newcompany.com")
    print(f"   Saving email change event...")
    ...
    for user_id, user_data in all_users.items():
        print(f"   - {user_data['name']} ({user_data['email']}) - registered: {user_data['registered_at']}")
    ...
    email_subscription = (SubscriptionBuilder()
    ...
    another_email_change = UserEmailChanged("user-2", "bob@example.com", "bob.johnson@newcompany.com")
    ...
    try:
        ...
        print(f"   Received filtered event: {filtered_event.event.event_type} for {filtered_event.event.data['user_id']}")
