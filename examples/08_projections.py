...
class ShoppingSession:
    ...
    def purchase_product(self, product_id: str, category: str, price: float, quantity: int = 1):
...
class UserActivityProjection:
    ...
    def apply_event(self, event: Event) -> bool:
        ...
        if self.category_views:
            self.favorite_category = max(self.category_views.items(), key=lambda x: x[1])[0]
...
class CategorySalesProjection:
    ...
    def __init__(self):
        self.categories: Dict[str, Dict[str, Any]] = defaultdict(lambda: {
            "total_views": 0,
            "total_purchases": 0,
            "total_revenue": 0.0,
            "unique_viewers": set(),
    ...
    def apply_event(self, event: Event) -> bool:
        ...
        if isinstance(event, ProductViewed):
            ...
            if cat_data["total_purchases"] > 0:
                cat_data["avg_price"] = cat_data["total_revenue"] / cat_data["total_purchases"]
    ...
    def get_category_summary(self, category: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "conversion_rate": round((len(cat_data["unique_buyers"]) / max(len(cat_data["unique_viewers"]), 1)) * 100, 2)
        }
...
class RealTimeDashboard:
    ...
    def apply_event(self, event: Event) -> bool:
        ...
        if isinstance(event, ProductViewed):
            ...
            if self.metrics["total_purchases_today"] > 0:
                self.metrics["avg_order_value"] = self.metrics["total_revenue_today"] / self.metrics["total_purchases_today"]
            
            self._add_recent_activity(event.user_id, "purchased", event.product_id, revenue)
            updated = True
            ...
            self._add_recent_activity(event.user_id, "placed_order", event.order_id, event.total_amount)
            updated = True
    ...
    def _add_recent_activity(self, user_id: str, action: str, item_id: str, value: float = None):
        ...
        if len(self.metrics["recent_activities"]) > self.recent_activity_limit:
            self.metrics["recent_activities"] = self.metrics["recent_activities"][:self.recent_activity_limit]
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        cart_to_order_rate = (funnel["orders_placed"] / max(funnel["cart_additions"], 1)) * 100
        order_completion_rate = (funnel["orders_completed"] / max(funnel["orders_placed"], 1)) * 100
...
class ProjectionManager:
    ...
    async def process_event(self, event: Event) -> Dict[str, bool]:
        ...
        if hasattr(event, 'user_id'):
            ...
            results["user_projection"] = self.user_projections[user_id].apply_event(event)
...
async def demonstrate_projections():
    ...
    for user_id in ["alice", "bob", "charlie"]:
        ...
        user.apply(UserRegistered(name=f"{user_id.title()} Johnson", email=f"{user_id}@example.com"))
    ...
    for event in all_events:
        results = await projection_manager.process_event(event)
    ...
    print(f"   ✓ Processing rate: {(len(all_events) / (processing_time / 1000)):.0f} events/sec")
    ...
    for user_id in ["alice", "bob", "charlie"]:
        ...
        if summary:
            ...
            print(f"     - Views: {summary['total_views']}, Purchases: {summary['total_purchases']}")
    ...
    for cat_summary in category_summaries:
        ...
        print(f"     - Views: {cat_summary['total_views']}, Purchases: {cat_summary['total_purchases']}")
    ...
    for metric, value in summary.items():
        formatted_value = f"${value}" if "revenue" in metric or "value" in metric else value
    ...
    print(f"     - Cart Additions: {funnel['cart_additions']} ({rates['view_to_cart']}% conversion)")
    print(f"     - Orders Placed: {funnel['orders_placed']} ({rates['cart_to_order']}% conversion)")
    print(f"     - Orders Completed: {funnel['orders_completed']} ({rates['order_completion']}% conversion)")
    ...
    print(f"     - Events per second: {(stats['processed_events'] / (processing_time / 1000)):.0f}")
    ...
    alice_specific_events = [e for e in all_events if hasattr(e, 'user_id') and e.user_id == "alice"]
    ...
    print(f"   Rebuild Results for Alice:")
    print(f"     - Original total spent: ${original_alice['total_spent']}")
    print(f"     - Rebuilt total spent: ${rebuilt_alice['total_spent']}")
    print(f"     - Data consistency: {'✓ MATCH' if original_alice['total_spent'] == rebuilt_alice['total_spent'] else '❌ MISMATCH'}")
    print(f"     - Rebuild time: {rebuild_time:.1f}ms for {len(alice_specific_events)} events")
...
async def main():
    ...
    print(f"- ✓ Single entity projections (user activity)")
    print(f"- ✓ Analytical projections (category analytics)")
    print(f"- ✓ Composite projections (real-time dashboard)")
    print(f"- ✓ Event-driven projection updates")
    print(f"- ✓ Real-time metrics and conversion funnels")
    print(f"- ✓ Projection rebuilding from event replay")
    print(f"- ✓ Query-optimized read models")
    ...
    print(f"- Processing rate: {(stats['processed_events'] / (processing_time / 1000)):.0f} events/sec")
