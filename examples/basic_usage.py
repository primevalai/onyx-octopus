...
def main():
    ...
    email_change_event = UserEmailChanged(
        old_email="alice@example.com", new_email="alice.johnson@company.com"
    )
    print(
        f"✓ UserEmailChanged: {email_change_event.old_email} → {email_change_event.new_email}"
    )
    ...
    print("✓ Applied registration event")
    print(f"  Name: {user.name}, Email: {user.email}")
    print(
        f"  Version: {user.version}, Uncommitted events: {len(user.get_uncommitted_events())}"
    )

    # Use business method (which generates and applies events)
    user.change_email("alice.johnson@newcompany.com")
    print("✓ Changed email via business method")
    ...
    print("✓ Applied deactivation event")
...
async def async_demo():
    ...
    try:
        ...
        await EventStore.create("sqlite://:memory:")
