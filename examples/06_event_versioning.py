...
class EventVersioningService:
    ...
    def upcast_event(event_data: Dict[str, Any]) -> Event:
        ...
        if event_type == "CustomerRegisteredV1" or (event_type == "CustomerRegistered" and event_version == 1):
            return EventVersioningService._upcast_customer_registered_v1_to_v2(event_data)
        
        elif event_type == "CustomerRegisteredV2" or (event_type == "CustomerRegistered" and event_version == 2):
            return EventVersioningService._upcast_customer_registered_v2_to_v3(event_data)
        
        # CustomerEmailChanged evolution
        elif event_type == "CustomerEmailChangedV1" or (event_type == "CustomerEmailChanged" and event_version == 1):
            return EventVersioningService._upcast_email_changed_v1_to_v2(event_data)
        
        elif event_type == "CustomerEmailChangedV2" or (event_type == "CustomerEmailChanged" and event_version == 2):
    ...
    @staticmethod
    def _upcast_customer_registered_v1_to_v2(v1_data: Dict[str, Any]) -> CustomerRegisteredV2:
    ...
    @staticmethod
    def _upcast_customer_registered_v2_to_v3(v2_data: Dict[str, Any]) -> CustomerProfileUpdatedV3:
    ...
    @staticmethod
    def _upcast_email_changed_v1_to_v2(v1_data: Dict[str, Any]) -> CustomerEmailChangedV2:
    ...
    @staticmethod
    def _upcast_email_changed_v2_to_v3(v2_data: Dict[str, Any]) -> CustomerProfileUpdatedV3:
        """Upcast CustomerEmailChangedV2 to V3."""
        
        profile_changes = {
            "email": v2_data.get("new_email", "")
        }
    ...
    def _create_event_from_data(event_data: Dict[str, Any]) -> Event:
        ...
        if "CustomerRegistered" in event_type:
            if "first_name" in event_data:
                return CustomerRegisteredV2(**{k: v for k, v in event_data.items() if k in CustomerRegisteredV2.__fields__})
            else:
                return CustomerRegisteredV1(**{k: v for k, v in event_data.items() if k in CustomerRegisteredV1.__fields__})
        
        elif "CustomerEmailChanged" in event_type:
            if "old_email" in event_data:
                return CustomerEmailChangedV2(**{k: v for k, v in event_data.items() if k in CustomerEmailChangedV2.__fields__})
            else:
                return CustomerEmailChangedV1(**{k: v for k, v in event_data.items() if k in CustomerEmailChangedV1.__fields__})
        
        elif "CustomerProfileUpdated" in event_type:
            return CustomerProfileUpdatedV3(**{k: v for k, v in event_data.items() if k in CustomerProfileUpdatedV3.__fields__})
...
class VersionedCustomer(Aggregate):
    ...
    def register_v2(self, first_name: str, last_name: str, email: str, phone: str = None, preferred_language: str = "en"):
    ...
    def change_email_v2(self, new_email: str, validated: bool = False, reason: str = None):
...
async def demonstrate_event_versioning():
    ...
    customer_v2.change_email_v2("jane.smith@company.com", validated=True, reason="Company email migration")
    ...
    for customer_id, format_name in customers:
        try:
            ...
            if events:
                ...
                print(f"       Registration: V{customer.registration_version}, Validated: {customer.email_validated}")
...
async def main():
    ...
    print(f"- ✓ Multiple event schema versions (V1, V2, V3)")
    print(f"- ✓ Event upcasting for forward compatibility")
    print(f"- ✓ Backward compatibility with legacy events")
    print(f"- ✓ Schema evolution with breaking changes")
    print(f"- ✓ Event migration and transformation")
    print(f"- ✓ Unified event handling across versions")
    print(f"- ✓ Gradual system migration strategies")
