...
class OrderProcessingSaga:
    ...
    def request_inventory_reservation(self, reservation_id: str):
        ...
        if self.state != SagaState.PAYMENT_COMPLETED:
            raise ValueError(
                f"Cannot request inventory reservation in state {self.state}"
            )
    ...
    def complete_shipping(self, tracking_number: str):
        ...
        event = ShippingCompleted(
            shipping_id=self.shipping_id, tracking_number=tracking_number
        )
        self._apply_event(event)
    ...
    def _start_compensation(self, failure_reason: str):
        ...
        event = OrderSagaFailed(
            reason=failure_reason, compensation_actions=self.compensation_actions
        )
        self._apply_event(event)
    ...
    def _compensate_shipping(self):
        ...
        event = ShippingCancelled(
            shipping_id=self.shipping_id, reason="Order processing failed"
        )
        self._apply_event(event)
...
class SagaOrchestrator:
    ...
    async def execute_saga(
        self, saga: OrderProcessingSaga, simulate_failures: Dict[str, bool] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if simulate_failures.get("payment", False):
                ...
                result["steps"].append(
                    f"Inventory reservation requested: {reservation_id}"
                )
...
async def demonstrate_saga_patterns():
    ...
    saga1 = await orchestrator.start_order_processing_saga(
        "order-001", "alice", "laptop", 1, 1200.0
    )
    ...
    saga2 = await orchestrator.start_order_processing_saga(
        "order-002", "alice", "mouse", 2, 100.0
    )
    ...
    saga3 = await orchestrator.start_order_processing_saga(
        "order-003", "alice", "keyboard", 1, 150.0
    )
    ...
    saga4 = await orchestrator.start_order_processing_saga(
        "order-004", "alice", "monitor", 1, 300.0
    )
    ...
    sagas = []
    for i in range(3):
        saga = await orchestrator.start_order_processing_saga(
            f"order-00{i+5}", "alice", "headset", 1, 80.0
        )
        sagas.append(saga)
    ...
    for i, result in enumerate(results):
        ...
        print(
            f"     - Order {result['order_id']}: {status} ({result['execution_time_ms']}ms)"
        )
    ...
    total_compensation_actions = sum(
        len(r["compensation_actions"]) for r in all_results
    )
    avg_execution_time = sum(r["execution_time_ms"] for r in all_results) / len(
        all_results
    )
...
async def main():
    ...
    print("- ✓ Orchestrated saga coordination")
    print("- ✓ Multi-step distributed transactions")
    print("- ✓ Compensating actions for failure recovery")
    print("- ✓ State machine-based saga management")
    print("- ✓ Concurrent saga execution")
    print("- ✓ Event-driven workflow coordination")
    print("- ✓ Failure isolation and recovery patterns")
