...
class InsufficientFundsError(DomainError):
    ...
    def __init__(self, balance: float, requested: float):
        ...
        super().__init__(
            f"Insufficient funds: balance ${balance:.2f}, requested ${requested:.2f}"
        )
...
class BankAccount(Aggregate):
    ...
    def open_account(self, account_holder: str, initial_deposit: float):
        ...
        event = AccountOpened(
            account_holder=account_holder.strip(), initial_deposit=initial_deposit
        )
        self.apply(event)
...
async def demonstrate_validation_errors():
    ...
    print(
        f"   ✓ Account opened: {account.account_holder}, balance: ${account.balance:.2f}"
    )
...
async def demonstrate_event_store_errors():
    ...
    try:
        await EventStore.create("invalid://bad-connection")
        print("   ❌ Should have failed")
...
async def main():
    ...
    print("\n=== Summary ===")
    print("Test accounts created with various error scenarios")
    print(
        f"Account 1: {account1.account_holder} - ${account1.balance:.2f} ({account1.status})"
    )
    print(
        f"Account 2: {account2.account_holder} - ${account2.balance:.2f} ({account2.status})"
    )
    ...
    print("- ✓ State-dependent operation validation")
    print("- ✓ Recovery strategies (retry, graceful degradation)")
    print("- ✓ Event store connection error handling")
    print("- ✓ Aggregate invariant protection")
