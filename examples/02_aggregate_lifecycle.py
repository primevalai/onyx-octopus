...
class Order(Aggregate):
    ...
    # Business methods
    def create_order(self, customer_id: str, product_name: str, quantity: int, unit_price: float):
    ...
    def apply_order_item_added(self, event: OrderItemAdded):
        ...
        self.items.append(OrderItem(event.product_name, event.quantity, event.unit_price))
        self._recalculate_total()
...
async def demonstrate_lifecycle():
    ...
    print(f"   ✓ Initial item: {order.items[0].product_name} x{order.items[0].quantity}")
    ...
    print(f"   ✓ Added mouse and keyboard")
...
async def main():
    ...
    print(f"\n=== Summary ===")
    print(f"Completed order: {completed_order.id} - ${completed_order.total_amount:.2f} ({completed_order.status})")
    print(f"Cancelled order: {cancelled_order.id} - ${cancelled_order.total_amount:.2f} ({cancelled_order.status})")
    
    print(f"\n✅ SUCCESS! Aggregate lifecycle patterns demonstrated!")
    
    print(f"\nKey patterns demonstrated:")
    print(f"- ✓ Complex aggregate state management")
    print(f"- ✓ Business rule enforcement at state transitions")
    print(f"- ✓ Event-driven state changes")
    print(f"- ✓ Aggregate versioning and consistency")
    print(f"- ✓ Multiple lifecycle paths (completion vs cancellation)")
    print(f"- ✓ Value objects and calculated fields")
    print(f"- ✓ Invariant protection")
